{ config, lib, ... }:
{
  ...
  config = lib.mkMerge [
    (lib.mkIf config.hotwire.enable { hotwire.formatter.enable = lib.mkDefault true; })
        ...
        let
          ...
          formatterPath =
            if builtins.pathExists dirPath then
              ...
              null;
        in
        lib.mkIf (formatterPath != null) { formatter = pkgs.callPackage formatterPath self'.packages; };
    })
  ];
}

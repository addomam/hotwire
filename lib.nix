...
{lib}: rec {
  ...
  */
        ...
        (lib.attrsets.filterAttrs (_: filePath: lib.strings.hasSuffix ".nix" (builtins.toString filePath))
          (builtins.mapAttrs
            (fileName: fileType:
              ...
              then directory + "/${fileName}/default.nix"
  ...
  Example:
    ...
    => { a = 1; b = 2; c = 3; }
  */
  recursiveUpdateAttr = attr: attrsets:
    lib.lists.foldl lib.attrsets.recursiveUpdate {} (
      builtins.map (attrset: attrset."${attr}")
      (builtins.filter (attrset: attrset ? "${attr}") attrsets)
    );
  ...
  Exaample:
    ...
    => [{...} {...}]
  */
  filterInputsForAttr = attr: inputs:
    lib.attrsets.mapAttrsToList (_: builtins.getAttr attr) (
      lib.attrsets.filterAttrs
      (
        name: input:
          name
          != "self" # filter out self to avoid infinite recursion
  ...
  */
  mkLib = {
    ...
    flakeLib =
      if libDir == null
      then {}
      else builtins.mapAttrs (_: file: import file {lib = inputsLib;}) (nixFiles libDir);
  in
  ...
  */
  mkFlake = args @ {
    ...
    # real mkFlake
    filteredArgs = lib.attrsets.filterAttrs (name: _:
      ! builtins.elem name [
        "libDir"
        "flakeModulesDir"
  ...
  in
    module:
      lib.mkFlake (lib.attrsets.recursiveUpdate filteredArgs {
        specialArgs.lib = lib;
      }) {
        imports =
          [
          ...
          ++ (
            if flakeModulesDir != null
            then builtins.attrValues (nixFiles flakeModulesDir)
            else []
          )
          ++ (
            if importFlakeInputModules # This is difficult to implement since flake-parts outputs shit modules
            then []
            else []
          );
      };
